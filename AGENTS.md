...
Worktrees are optional. When using one, keep each task on its own dedicated branch and avoid
switching branches in a checkout another session is actively using.
...
19. Do not switch branches in a checkout another session is actively using. Worktrees are optional; when used, keep each task on its own dedicated branch and leave other sessions' worktrees untouched.
